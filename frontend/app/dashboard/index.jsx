...
const DashboardPage = ({ onPageChange }) => {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      ...
      // Poll for balance updates every 5 seconds (silent mode - no loading spinner)
      const interval = setInterval(() => {
        fetchDashboardData(true); // true = silent mode
      }, 5000);
        ...
        <p className="text-gray-400">Here's your gaming overview</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-400 text-sm">{t('dashboard.balance')}</p>
                      <p className="text-3xl font-bold text-white">{stats.balance.toFixed(2)} AXION</p>
                  <p className="text-green-400 text-xs mt-1">≈ ${(stats.balance * 1.0).toFixed(2)} USD</p>
                </div>
          ...
          <div className="glass-effect rounded-xl p-6 mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <button 
                onClick={() => onPageChange && onPageChange('games')} 
                    ...
                    <li key={game.id} className="flex justify-between items-center text-gray-300 hover:text-white transition-colors">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <span className="text-sm text-gray-400 ml-2">{formatDate(game.date)}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className={`font-bold ${getResultColor(game.result)}`}>
                            {game.result === 'Win' ? '+' : '-'}{game.amount.toFixed(2)} AXION
                        </span>
                        {game.multiplier > 0 && (
                    ...
                    <li key={transaction.id} className="flex justify-between items-center text-gray-300 hover:text-white transition-colors">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <span className="text-sm text-gray-400 ml-2">{formatDate(transaction.date)}</span>
                      ...
                      <div className="text-right">
                        <span className={`font-bold ${transaction.type === 'Deposit' ? 'text-green-400' : 'text-red-400'}`}>
                            {transaction.type === 'Deposit' ? '+' : '-'}{transaction.amount.toFixed(2)} AXION
                        </span>
