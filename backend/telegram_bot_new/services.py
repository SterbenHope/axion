...
Команды работают в админском чате"""
                    ...
                    # Check if in manager chat or private message
                    if message.chat.type == 'private':
                        help_text = """🎯 Команды менеджера (личные сообщения):
...
/create_promo <код> - Создать промокод
  • Пример: /create_promo WELCOME2024
  • Автоматически: +30% бонус, 365 дней
  • Получите ссылку для регистрации
  
/list_promos - Список ваших промокодов
  • Последние 10 промокодов с количеством активаций
  
/my_stats - Ваша статистика и заработок
  • Количество промокодов и активаций
  • Сумма депозитов и ваш заработок (50%)
  • Уникальные пользователи

Как работать:
1. Создайте промокод командой /create_promo
2. Скопируйте ссылку из ответа
3. Поделитесь ссылкой с клиентами
4. Отслеживайте статистику через /my_stats"""
                    else:
                        help_text = """🎯 Команды менеджера (чат менеджеров):
...
Функции менеджера:
• Создание промокодов с автоматической ссылкой
• Просмотр статистики по всем промокодам
• Отслеживание заработка (50% от депозитов)
...
• Общая выручка: {usage.assigned_manager.total_revenue_generated} EUR
    ...
    async def handle_my_stats(self, message):
        ...
        try:
            ...
            # If no linked_user, try to find by telegram username or bot_user
            if user_obj:
                promo_codes = await sync_to_async(list)(PromoCode.objects.filter(created_by=user_obj))
            else:
                # If no linked user, check by name or return empty
                promo_codes = []
